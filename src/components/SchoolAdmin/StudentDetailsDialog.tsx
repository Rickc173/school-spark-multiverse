...
const StudentDetailsDialog = ({ student, isOpen, onClose }: StudentDetailsDialogProps) => {
  ...
  const handleSettleInvoice = (invoiceId: number, paymentData: any) => {
    console.log('Settling invoice:', invoiceId, paymentData);
    setInvoices(invoices.map(inv => 
      inv.id === invoiceId 
        ? { ...inv, status: 'Paid', paidDate: paymentData.paymentDate }
        : inv
    ));
  };
                      ...
                      {invoices.filter(inv => inv.status !== 'Paid').map((invoice) => (
                        <div key={invoice.id} className="flex items-center justify-between border rounded p-3">
                          ...
                          <div className="flex space-x-2">
                            <Button size="sm" variant="outline" onClick={() => handleViewInvoiceDetails(invoice)}>
                              View Details
                            </Button>
                            <Button size="sm" onClick={() => handleViewInvoiceDetails(invoice)}>
                              Settle
                            </Button>
                          </div>
                        </div>
